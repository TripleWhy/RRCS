	...
	public class ShareFileModal : Modal
	{
		...
		public IEnumerator Show(string data)
		{
			...
			int leftSideBarWidth =
				(int) Math.Ceiling(layoutManager.GetAdjustedLeftSidebarWidth() +
				                   layoutManager.GetCollapseButtonWidth());
			int rightSideBarWidth =
				(int) Math.Ceiling(
					layoutManager.GetAdjustedRightSidebarWidth() + layoutManager.GetCollapseButtonWidth());
